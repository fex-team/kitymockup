...
kity.extendClass(Mockup, {
    ...
    _firePharse: function(e) {
        ...
        beforeEvent = new MockupEvent('before' + e.type, e, true);
        if (this._fire(beforeEvent)) {
            return;
        }
        preEvent = new MockupEvent('pre' + e.type, e, true);
        executeEvent = new MockupEvent(e.type, e, true);

        this._fire(preEvent) ||
            this._fire(executeEvent) ||
            this._fire(new MockupEvent('after' + e.type, e, false));
    ...
    _interactChange: function(e) {
        ...
        this._interactTimeout = setTimeout(function() {
            var stoped = minder._fire(new MockupEvent('beforeinteractchange'));
            if (stoped) {
                return;
            }
            minder._fire(new MockupEvent('preinteractchange'));
            minder._fire(new MockupEvent('interactchange'));
        }, 20);
    },
    ...
    fire: function(type, params) {
        var e = new MockupEvent(type, params);
        this._fire(e);
        return this;
    }
});